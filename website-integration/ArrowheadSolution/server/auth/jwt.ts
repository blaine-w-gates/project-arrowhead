...
export function verifyJwt(token: string, secret: string): { valid: boolean; payload?: any; reason?: string } {
  try {
    ...
    const payload = JSON.parse(Buffer.from(payloadPart, 'base64').toString('utf8')) as any;
    const now = Math.floor(Date.now() / 1000);
    if (typeof payload.exp === 'number' && payload.exp < now) return { valid: false, reason: 'exp' };
