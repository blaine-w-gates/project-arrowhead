...
// Keep test-specific helpers
async function createVirtualMember(page: any, teamId: string, memberName: string): Promise<string> {
  const response = await page.evaluate(async ({ teamId, memberName }: any) => {
    const res = await fetch(`/api/teams/${teamId}/members`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: memberName, isVirtual: true }),
    });
...
async function getTeamId(page: any): Promise<string> {
...
async function inviteMemberViaAPI(page: any, memberId: string, email: string): Promise<{ status: number; data: unknown }> {
  return await page.evaluate(async ({ memberId, email }: any) => {
