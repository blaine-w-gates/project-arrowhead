...
// Group all blog E2E tests under a skipped suite for Sprint 2
test.describe.skip('Blog E2E', () => {
  // List View Test
  // Asserts that the titles of the published sample posts appear on the /blog page
  test('Blog list view shows published sample posts', async ({ page }) => {
    await page.goto('/blog');
    await expect(
      page.getByRole('heading', { level: 1, name: /Strategic Insights Blog/i })
    ).toBeVisible();
  ...
  test('Blog detail view renders title and markdown content', async ({ page }) => {
    ...
    // Click the article card link from the list view
    ...
    await expect(
      page.getByRole('heading', {
        level: 1,
        name: 'Beyond the Buzzwords: The First Question Every Team Must Answer Before Brainstorming',
      })
    ).toBeVisible();
